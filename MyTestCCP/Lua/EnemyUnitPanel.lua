...
-- Be sure to reload the database, because the read-only one from GameInfo.HandicapInfos is out of date
if Game then
		...
		if (iMyStrength > 0) then
			...
			if not pTheirPlayer:IsBarbarian() and not pTheirPlayer:IsHuman() then
				if (bRanged) then
					...
					-- iModifier = iModifier + pMyPlayer:GetTraitGreatGeneralExtraBonus();
					controlTable = g_MyCombatDataIM:GetInstance();
					controlTable.Text:SetText( "Range attack vs AI" );
					controlTable.Value:SetText( GetFormattedText(strText, - DB_HandicapInfos[pMyPlayer:GetHandicapType()].RangeHumanVsAIAtkDmgMultPercent, true, true) );
				else
					-- iModifier = GameDefines["GREAT_GENERAL_STRENGTH_MOD"];
					-- iModifier = iModifier + pMyPlayer:GetTraitGreatGeneralExtraBonus();
					controlTable = g_MyCombatDataIM:GetInstance();
					controlTable.Text:LocalizeAndSetText( "Melee attack vs AI" );
					controlTable.Value:SetText( GetFormattedText(strText, - DB_HandicapInfos[pMyPlayer:GetHandicapType()].MeleeHumanVsAIAtkDmgMultPercent, true, true) );
				end
			end
			-- END MOD_COMBAT_HANDICAP	
...
function UpdateCombatOddsCityVsUnit(myCity, theirUnit)
	...
	local iMyPlayer = myCity:GetOwner();
	local pMyPlayer = Players[iMyPlayer];
